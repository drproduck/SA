...
# test to see if it can learn an exponential model with binary covariate and censoring
...
rate_cens = 0.002
n_samples = 5000
t1 = np.random.exponential(scale=1. / rate1, size=int(n_samples/2))
t2 = np.random.exponential(scale=1. / rate2, size=int(n_samples/2))
t = np.concatenate((t1, t2))
group = np.zeros(n_samples)
group[int(n_samples/2):] = 1

censtime = np.random.exponential(scale=1. / rate_cens, size=n_samples)
e = t < censtime
t[~e] = censtime[~e]
print(f'{sum(e)}/{n_samples} events observed.')

kmf = KaplanMeierFitter()
kmf.fit(t[0:int(n_samples/2)], event_observed=e[0:int(n_samples/2)])
plt.plot(kmf.survival_function_.index.values, kmf.survival_function_.KM_estimate, alpha=0.5, label='km-group-1')
kmf.fit(t[int(n_samples/2):], event_observed=e[int(n_samples/2):])
plt.plot(kmf.survival_function_.index.values, kmf.survival_function_.KM_estimate, alpha=0.5, label='km-group-2')
plt.xticks(np.arange(0, 2000.0001, 200))
plt.yticks(np.arange(-0.125, 1.0001, 0.125))
plt.xlim([0,2000])
plt.ylim([-0.125,1])
plt.xlabel('Follow-up time (days)')
plt.ylabel('Proportion surviving')
plt.title('Simple exponential model with censoring and covariate')
plt.plot([0, 2000], [0, 0], linestyle='dashed', alpha=0.5)
# plt.show()
...
plt.plot(sample_t, curve1, alpha=0.5, linestyle='-.', label='ground-truth-group-1')
plt.plot(sample_t, curve2, alpha=0.5, linestyle='-.', label='ground-truth-group-2')
...
dataset1 = TensorDataset(torch.from_numpy(x_train).float(), torch.from_numpy(t).unsqueeze(1).float(), torch.from_numpy(e).unsqueeze(1).float())
train_loader = torch.utils.data.DataLoader(dataset1, batch_size=args.batch_size, shuffle=True)

model = Simple(x_dim=2).to(device)
print(model)
...
for t in np.linspace(0, 2000, 100):
    survival_rate, hazard_rate = predict_survival_at_t(model, torch.Tensor([[1., 0.]]), t / t_max)
    # plt.plot(t, survival_rate, 'rx', markersize=5)
    list_survival_rate.append(survival_rate)
plt.plot(np.linspace(0, 2000, 100), list_survival_rate, markersize=5, linestyle='--', label='nn-group-1')

list_survival_rate = []
for t in np.linspace(0, 2000, 100):
    survival_rate, hazard_rate = predict_survival_at_t(model, torch.Tensor([[0., 1.]]), t / t_max)
    # plt.plot(t, survival_rate, 'rx', markersize=5)
    list_survival_rate.append(survival_rate)
plt.plot(np.linspace(0, 2000, 100), list_survival_rate, markersize=5, linestyle='--', label='nn-group-2')
...
print('t_max: ', t_max)
print(hazard_rate / t_max)
print(t_max)

plt.legend()
plt.savefig('test2_3.png')